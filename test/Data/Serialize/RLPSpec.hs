...
spec = do
  context "when encoding _empty_ values" $ do
    ...
    it "should decode it back" $ do
      (rlpDecode . rlpEncode . toByteStringS $ "" :: DBS.ByteString) `shouldBe` (toByteStringS "")

    it "should encode the empty list" $ do
      (DBSL.unpack . rlpEncode . RLPL $ []) `shouldBe` [ 0xc0 ]

    it "should decode it back" $ do
      (rlpDecode . rlpEncode . RLPL $ [] :: RLPT) `shouldBe` (RLPL []) 

    it "should encode the integer Zero" $ do
      (DBSL.unpack . rlpEncode . toRLP $ (0 :: Int)) `shouldBe` [ 0x80 ]

    it "should decode it back" $ do
      (fromRLP . rlpDecode . rlpEncode . toRLP $ (0 :: Int) :: Int) `shouldBe` 0
  ...
  context "when encoding ByteArrays" $ do
    ...
    it "should decode it back" $ do
      (rlpDecode . rlpEncode . toByteStringS $ "\NUL" :: DBS.ByteString) `shouldBe` (toByteStringS "\NUL")

    it "should encode an array of length 1 and big value" $ do
      (DBSL.unpack . rlpEncode . toByteStringS $ "\150") `shouldBe` [ 0x81, 0x96 ]

    it "should decode it back" $ do
      (rlpDecode . rlpEncode . toByteStringS $ "\150" :: DBS.ByteString) `shouldBe` (toByteStringS "\150")
    ...
    it "should decode it back" $ do
      (rlpDecode . rlpEncode . toByteStringS $ "\SOH\SOH" :: DBS.ByteString) `shouldBe` (toByteStringS "\SOH\SOH")
    ...
    it "should decode it back" $ do
      (rlpDecode . rlpEncode . toByteStringS . Prelude.map toEnum . Prelude.take 60 . Prelude.repeat $ 1 :: DBS.ByteString)
        `shouldBe` (toByteStringS . Prelude.map toEnum . Prelude.take 60 . Prelude.repeat $ 0x01)
    ...
    it "should decode it back" $ do
      (rlpDecode . rlpEncode . toByteStringS . Prelude.map toEnum . Prelude.take 43520 . Prelude.repeat $ 1 :: DBS.ByteString)
        `shouldBe` (toByteStringS . Prelude.map toEnum . Prelude.take 43520 . Prelude.repeat $ 0x01)
  ...
  context "when encoding RLP structures" $ do
    ...
    it "should decode it back" $ do
      (rlpDecode . rlpEncode $ RLPL [ RLPB . toByteStringS $ "\NUL" ] :: RLPT)
        `shouldBe` (RLPL [ RLPB . toByteStringS $ "\NUL" ])
    ...
    it "should decode it back" $ do
      (rlpDecode . rlpEncode $ RLPL [ RLPB . toByteStringS $ "\NUL" , RLPL [ RLPB . toByteStringS $ "\NUL" ] ] :: RLPT)
        `shouldBe` (RLPL [ RLPB . toByteStringS $ "\NUL" , RLPL [ RLPB . toByteStringS $ "\NUL" ] ])
    ...
    it "should decode it back" $ do
      (rlpDecode . rlpEncode $ RLPL [ RLPB . toByteStringS . Prelude.map toEnum . Prelude.take 60 . Prelude.repeat $ 1 ] :: RLPT)
        `shouldBe` (RLPL [ RLPB . toByteStringS . Prelude.map toEnum . Prelude.take 60 . Prelude.repeat $ 1 ])
    ...
    it "should decode it back" $ do
      (rlpDecode . rlpEncode $ RLPL [ RLPB . toByteStringS $ "\NUL" , RLPL [ RLPB . toByteStringS . Prelude.map toEnum . Prelude.take 60 . Prelude.repeat $ 1 ] ] :: RLPT)
        `shouldBe` (RLPL [ RLPB . toByteStringS $ "\NUL" , RLPL [ RLPB . toByteStringS . Prelude.map toEnum . Prelude.take 60 . Prelude.repeat $ 1 ] ])
  ...
  context "when running Ethereum examples" $ do
    ...
    it "should decode it back" $ do
      (rlpDecode . rlpEncode . toByteStringS $ "dog" :: DBS.ByteString) `shouldBe` (toByteStringS "dog")
    ...
    it "should decode it back" $ do
      (rlpDecode . rlpEncode . RLPL . Prelude.map (RLPB . toByteStringS) $ [ "cat", "dog" ] :: RLPT)
        `shouldBe` (RLPL . Prelude.map (RLPB . toByteStringS) $ [ "cat", "dog" ])
    ...
    it "should encode it back" $ do
      (rlpDecode . rlpEncode . toRLP $ RLPL [ RLPL [] , RLPL [ RLPL [] ] , RLPL [ RLPL [] , RLPL [ RLPL [] ] ] ] :: RLPT)
        `shouldBe` (RLPL [ RLPL [] , RLPL [ RLPL [] ] , RLPL [ RLPL [] , RLPL [ RLPL [] ] ] ])
    ...
    it "should encode it back" $ do
      (rlpDecode . rlpEncode . toByteStringS $ "Lorem ipsum dolor sit amet, consectetur adipisicing elit" :: DBS.ByteString)
        `shouldBe` (toByteStringS $ "Lorem ipsum dolor sit amet, consectetur adipisicing elit")
